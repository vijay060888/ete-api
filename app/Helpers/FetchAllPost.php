...
class FetchAllPost
{
    ...
    public static function getAllPost($currentPage, $partyId, $keyword, $activity, $archieve, $createdBy, $postByFilter, $trending = null)
    {
        $sources = [
            // 'getAds',
            // 'getAllFromFactCheck',
        ...
        if($archieve){
            if($trending){
                $combinedPosts = collect($combinedPosts)->unique('postId');
                $combinedPosts = $combinedPosts->sortBy(function($post) use ($archieve) {
                return array_search($post['postId'], $archieve);
                });
            }else {
